...
function board_modify(e) {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.post(contextPath3 + 'Admin/board/AdminUserBoardModify_pro', $('#adminUserBoardBean').serialize(), function(response) {
				Swal.fire({
					title: "게시글 수정",
					text: "게시글이 수정 되었습니다.",
					icon: "success"
...
function board_delete(e, user_board_num) {
	e.preventDefault();
	Swal.fire({
		title: "게시글 삭제",
		text: "게시글을 삭제 하시겠습니까?",
		...
		if (result.isConfirmed) {
			$.post(contextPath3 + 'Admin/board/AdminUserBoardDelete', { user_board_num: user_board_num }, function(response) {
...
function board_modify_cancel(e) {
	e.preventDefault();
	Swal.fire({
		title: "게시글 수정",
